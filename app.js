(function(){
  ...
  function fmtMoney(n){ var sign = n<0 ? "-" : ""; var v = Math.abs(n||0); return sign + "$" + v.toLocaleString('es-CO',{maximumFractionDigits:0}); }
  ...
  var DEFAULTS={
    ...
    savePercent:20, transactions:[], debts:[]
  };
  ...
  function renderResumen(){ var now=new Date(); var m=sumPeriod(startOfMonth(now), endOfMonth(now)), w=sumPeriod(startOfWeek(now), endOfWeek(now)), t=sumPeriod(new Date(now.toDateString()), new Date(now.toDateString())); $('#mIncome').textContent=fmtMoney(m.income); $('#mExpense').textContent=fmtMoney(m.expense); $('#mBalance').textContent=fmtMoney(m.balance); $('#wIncome').textContent=fmtMoney(w.income); $('#wExpense').textContent=fmtMoney(w.expense); $('#wBalance').textContent=fmtMoney(w.balance); $('#tIncome').textContent=fmtMoney(t.income); $('#tExpense').textContent=fmtMoney(t.expense); $('#tBalance').textContent=fmtMoney(t.balance); var min=state.debts.reduce(function(a,b){return a+(Number(b.min)||0);},0); $('#minDebt').textContent=fmtMoney(min); var goal=(state.savePercent/100)*(m.income||0); var current=Math.max(0,m.income-m.expense); $('#mSaveGoal').textContent=fmtMoney(goal); $('#mSaveNow').textContent=fmtMoney(current); var pct=goal?Math.min(100,Math.round(100*current/goal)):0; $('#mSaveBar').style.width=pct+'%'; var byCat={}, items; w.list.filter(function(x){return x.type==='Gasto';}).forEach(function(x){ byCat[x.category]=(byCat[x.category]||0)+x.amount; }); items=Object.keys(byCat).map(function(k){return [k,byCat[k]];}).sort(function(a,b){return b[1]-a[1];}).slice(0,5); $('#wCats').innerHTML=items.length?items.map(function(it){return "<div class='row'><span class='pill'>"+escapeHtml(it[0])+"</span><strong class='mono bad'>"+fmtMoney(it[1])+"</strong></div>";}).join(''):'<span class=\"muted\">Sin datos</span>'; var byCatM={}, totalG; m.list.filter(function(x){return x.type==='Gasto';}).forEach(function(x){ byCatM[x.category]=(byCatM[x.category]||0)+x.amount; }); totalG=m.expense||1; var html=Object.keys(byCatM).map(function(k){return [k,byCatM[k]];}).sort(function(a,b){return b[1]-a[1];}).map(function(pair){ var c=pair[0], v=pair[1], p=Math.round(100*v/totalG); return "<div class='row' style='justify-content:space-between;margin:6px 0'><div class='row' style='gap:8px'><span class='pill'>"+escapeHtml(c)+"</span> <span class='muted'>"+p+"% del gasto</span></div><strong class='mono bad'>"+fmtMoney(v)+"</strong></div><div class='bar' title='Participación mensual'><em style='width:"+p+"%'></em></div>"; }).join('') || '<span class=\"muted\">Sin datos</span>'; $('#catsBreak').innerHTML=html; }
  ...
  document.addEventListener('click', function(e){ var btn=e.target.closest?e.target.closest('button.delDebt'):null; if(!btn)return; var id=btn.getAttribute('data-id'); state.debts=state.debts.filter(function(x){return x.id!==id;}); save(); renderDebts(); renderResumen(); });
  ...
  $('#saveSettings').addEventListener('click',function(){ var p=Number($('#savePct').value||0); state.savePercent=Math.max(0,Math.min(100,p)); save(); renderResumen(); alert('Ajustes guardados'); });
  ...
  // ===== PWA: Instalación y SW =====
  // Mostrar botón "Instalar" con beforeinstallprompt
  ...
  // Registrar Service Worker (network-first)
