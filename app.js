(function(){
  ...
  function parseCSV(text){var sep=text.indexOf(';')>-1?';':',';var lines=text.split(/\r?\n/).filter(function(x){return x.trim().length>0});if(!lines.length)return[];var headers=lines.shift().split(sep).map(function(h){return h.trim().replace(/^"|"$/g,'')});var out=[];for(var li=0;li<lines.length;li++){var line=lines[li],cells=[],cur='',q=false;for(var i=0;i<line.length;i++){var ch=line[i];if(ch==='"'){q=!q;continue}if(ch===sep&&!q){cells.push(cur);cur='';continue}cur+=ch}cells.push(cur);var obj={};for(var ci=0;ci<headers.length;ci++){obj[headers[ci]]=(cells[ci]||'').trim()}out.push(obj)}return out}

  // ===== State =====
  var DEFAULTS={categories:["Fijos","Necesarios","Transporte","Alimentación","Salud","Educación","Ocio","Servicios","Otros","Sueldo","Honorarios"],
  ...
  function show(key){$$('#tabs .tab').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-key')===key)});PANELS.forEach(function(p){var el=document.getElementById('panel-'+p.key);if(el)el.hidden=(p.key!==key)});if(key==='resumen')renderResumen();if(key==='registrar'){renderTxTable();fillSelectors()}if(key==='deudas')renderDebts();if(key==='categorias')renderLists()}
  tabs.addEventListener('click',function(e){var btn=e.target.closest?e.target.closest('button.tab'):null;if(!btn)return;show(btn.getAttribute('data-key'))});show('resumen');

  // Online badge
  function updateOnline(){var b=$('#onlineBadge');if(!b)return;if(navigator.onLine){b.textContent='🟢 Online';b.classList.remove('offline');b.classList.add('online')}else{b.textContent='🔴 Offline';b.classList.remove('online');b.classList.add('offline')}}window.addEventListener('online',updateOnline);window.addEventListener('offline',updateOnline);updateOnline();

  // ===== Registrar =====
  var currentType='Gasto';$('#type-expense').addEventListener('click',function(){currentType='Gasto';$('#chosenType').textContent='Tipo: Gasto'});
  ...
  // Tabla
  function renderTxTable(){var q=($('#search').value||'').toLowerCase(),ty=$('#filterType').value||'all',m=$('#filterMonth').value;var rows=state.transactions.slice();if(q){rows=rows.filter(function(r){return(r.description+' '+r.category+' '+r.method).toLowerCase().indexOf(q)>-1})}if(ty!=='all'){rows=rows.filter(function(r){return r.type===ty})}if(m){rows=rows.filter(function(r){return r.date&&r.date.indexOf(m)===0})}var inc=0,exp=0;rows.forEach(function(r){if(r.type==='Ingreso')inc+=r.amount||0;else exp+=r.amount||0});$('#txTotals').innerHTML="<span class='pill'>Filtrados: <b>"+rows.length+"</b></span>"+"<span class='pill'>Ingresos: <b class='ok'>"+fmtMoney(inc)+"</b></span>"+"<span class='pill'>Gastos: <b class='bad'>"+fmtMoney(exp)+"</b></span>"+"<span class='pill'>Balance: <b>"+fmtMoney(inc-exp)+"</b></span>";var html="<tr><th>Fecha</th><th>Tipo</th><th>Descripción</th><th>Categoría</th><th>Método</th><th class='right'>Monto</th><th></th></tr>";rows.forEach(function(r){var sign=r.type==='Gasto'?-1:1;html+="<tr><td>"+(r.date||'')+"</td><td><span class='pill'>"+r.type+"</span></td><td>"+escapeHtml(r.description)+"</td><td>"+escapeHtml(r.category)+"</td><td>"+escapeHtml(r.method)+"</td><td class='right mono "+(sign<0?'bad':'ok')+"'>"+fmtMoney(sign*r.amount)+"</td><td class='right'><button data-id='"+r.id+"' class='ghost delTx'>Eliminar</button></td></tr>"});$('#txTable').innerHTML=html}
  $('#search').addEventListener('input',renderTxTable);$('#filterType').addEventListener('change',renderTxTable);$('#filterMonth').addEventListener('change',renderTxTable);
  ...
  function renderResumen(){var now=new Date();var m=sumPeriod(startOfMonth(now),endOfMonth(now)),w=sumPeriod(startOfWeek(now),endOfWeek(now)),t=sumPeriod(new Date(now.toDateString()),new Date(now.toDateString()));$('#mIncome').textContent=fmtMoney(m.income);$('#mExpense').textContent=fmtMoney(m.expense);$('#mBalance').textContent=fmtMoney(m.balance);$('#wIncome').textContent=fmtMoney(w.income);$('#wExpense').textContent=fmtMoney(w.expense);$('#wBalance').textContent=fmtMoney(w.balance);$('#tIncome').textContent=fmtMoney(t.income);$('#tExpense').textContent=fmtMoney(t.expense);$('#tBalance').textContent=fmtMoney(t.balance);var min=state.debts.reduce(function(a,b){return a+(Number(b.min)||0)},0);$('#minDebt').textContent=fmtMoney(min);var goal=(state.savePercent/100)*(m.income||0);var current=Math.max(0,m.income-m.expense);$('#mSaveGoal').textContent=fmtMoney(goal);$('#mSaveNow').textContent=fmtMoney(current);var pct=goal?Math.min(100,Math.round(100*current/goal)):0;$('#mSaveBar').style.width=pct+'%';var byCatW={};w.list.filter(function(x){return x.type==='Gasto'}).forEach(function(x){byCatW[x.category]=(byCatW[x.category]||0)+x.amount});var items=Object.entries(byCatW).sort(function(a,b){return b[1]-a[1]}).slice(0,5);$('#wCats').innerHTML=items.length?items.map(function(p){return"<div class='row'><span class='pill'>"+escapeHtml(p[0])+"</span><strong class='mono bad'>"+fmtMoney(p[1])+"</strong></div>"}).join(''):'<span class="muted">Sin datos</span>';var byCatM={};m.list.filter(function(x){return x.type==='Gasto'}).forEach(function(x){byCatM[x.category]=(byCatM[x.category]||0)+x.amount});var totalG=m.expense||1;var html=Object.entries(byCatM).sort(function(a,b){return b[1]-a[1]}).map(function(p){var c=p[0],v=p[1],pcent=Math.round(100*v/totalG);return"<div class='row' style='justify-content:space-between;margin:6px 0'><div class='row' style='gap:8px'><span class='pill'>"+escapeHtml(c)+"</span> <span class='muted'>"+pcent+"% del gasto</span></div><strong class='mono bad'>"+fmtMoney(v)+"</strong></div><div class='bar'><em style='width:"+pcent+"%'></em></div>"}).join('');if(!html)html='<span class="muted">Sin datos</span>';$('#catsBreak').innerHTML=html}
